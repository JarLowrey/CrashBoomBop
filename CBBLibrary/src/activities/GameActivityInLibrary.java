...
public class GameActivityInLibrary extends Activity implements OnClickListener, SensorEventListener
{
	//Resource ID arrays
    ...
    public int[] viewsThatPersistOnClearScreen = {R.id.heartGroup,R.id.score,R.id.pause_or_play};
    ...
    //view variables
    protected RelativeLayout gameWindow,detectTapArea,gameOverWindow,heartGroup;
	...
	private ScreenInteractions changeScreen;
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    gameOverWindow = (RelativeLayout)this.findViewById(R.id.gameOverWindow);
		...
		mSensorManager.registerListener(GameActivityInLibrary.this, mAccelerometer,SensorManager.SENSOR_DELAY_NORMAL);
...
//	    wrong_and_right_toast = new Toast(this);//should this be getApplicationContext?
		...
		gameOverWindow.setVisibility(View.GONE);
	...
	public void onSensorChanged(SensorEvent event) { //Algorithm to check sensors: current coord-previous coord. That's it.
		if(!gameIsPaused && gameStarted && !gameOver){//avoid uneccessary processing. wait until setup complete (flagged by gameStarted)
			if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
				...
				CaseResult result = checkCase.checkSensorForPassOrFail(x, y, z, myModel.getCurrentCase());
				...
				if(screenIsCleared){
					...
					if(myModel.currentCaseIsBonus()){
						...
						//bonus case not over, update screen
						else {
							bonusTimeRemaining.setText((endTime-watch.getElapsedTime())/1000+"");
							if(myModel.getCurrentCase() == Case.MANYSHAKESX /*|| myModel.getCurrentCase()==Case.MANYSHAKESY || myModel.getCurrentCase()==Case.MANYSHAKESZ*/){
								if(result == CaseResult.PASS){
	...
	public void onClick(View v) {
		int id = v.getId();
		if(id == R.id.gameOverAccept){
			leaveGame();
		}
		else if(id == R.id.gameOverShare){
			String[] sharingTargets = {"face","twit","whats","chat","sms","mms","plus","email","gmail"};
			changeScreen.findAndDisplayTargetedShareIntentChooser(this,myModel.getScore(),sharingTargets,
					"Crash Boom Bop","Check out my "+myModel.getScore()+" points! Beat that!! #CrashBoomBop");		
		}
		else if (id == R.id.pause_or_play){
			if(gameIsPaused){resumeGame();}
			else{pauseGame();}			
		}else{
			if( !gameIsPaused && !screenIsCleared && !handlerIsRunning){
				switch(myModel.getCurrentCase()){
				case TAP:	
...
//					toastCreator.createToast(failedCaseMessage, this, wrong_and_right_toast);
			...
			case PASS:
				...
				else if(!myModel.currentCaseIsBonus() && myModel.getCurrentCase()!=Case.TAP && myModel.getCurrentCase()!=Case.MANYTAPS && myModel.getCurrentCase()!=Case.MOVINGBTN ){
		...
		if(!gameOver){
	    	...
	    	changeScreen.showHeartsOnScreen(this,heartGroup,myModel.life);
			scorePanel.drawText(myModel.getScore()+"");
		}
	}
	...
	public void showNewCase(){
		...
		if(myModel.isCombo){
			...
			if(myModel.currentCaseIsBonus()){
				showAlertMsg(myModel.getCurrentCase().toString(),1000,bonus);
				switch(myModel.getCurrentCase()){
				case MOVINGBTN:
					phone.setVisibility(View.GONE);
					changeScreen.moveBtn(this,gameWindow,movingBtn);
					break;
				case MANYTAPS:
					setAnimationAndPhoneImage(myModel.getCurrentCase());
					break;
				case MANYSHAKESX:
					setAnimationAndPhoneImage(myModel.getCurrentCase());
					break;
	...
	protected void gameOver(){
		...
		gameOverWindow.setVisibility(View.VISIBLE);
		...
		//update gameStatsDb 
		gameStatsDb.addRow(myModel.getScore(), totalResponseTimeThisGame, totalTimeThisGame, 
		...
		medal.setBackgroundResource(changeScreen.chooseMedalId(this,myModel.getScore()));
...
//			if(wrong_and_right_toast!=null){wrong_and_right_toast.cancel();}
	...
	private void resumeGame(){		
		...
		//show user game has resumed by showing the screen, clearing alpha, playing the getCurrentCase() animation, and hiding pauseText
	...
	private void setAnimationAndPhoneImage(Case whichCase){
		...
		myAnim=AnimationUtils.loadAnimation(this,myModel.getCurrentCase().getAnimationId());
